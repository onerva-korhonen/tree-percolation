...
effective_conductances, lcc_sizes, functional_lcc_sizes, nonfunctional_component_size, susceptibilities, functional_susceptibilities = percolation.run_percolation(net_cleaned, cfg, percolation_type='bond', removal_order='random')
...
visualization.plot_percolation_curve(net_cleaned['throat.conns'].shape[0], np.expand_dims(nonfunctional_component_size, axis=0),
                                     colors=['b'], labels=['total nonfunctional component size'], alphas=[1], save_path=params.nonfunctional_componen_size_save_path)
